...
Процедура Синхронизировать(Знач КаталогРабочейКопии, Знач ПутьКХранилищу,
	...
	Пока Индекс >= 0 Цикл
		...
		Значение = МассвВерсий[Индекс];
		Лог.Информация(СтрШаблон("Обрабатываем версию :'%1'", значение));
		Индекс = Индекс - 1;
		...
		Для каждого Строка из ТаблицаИсторииХранилища Цикл 
			Если СтрНайти(Строка.Комментарий, Значение) Тогда
		...
		Комментарий  = Комментарий + Символы.ПС + "git2reposha:" + значение;
		Лог.Информация(СтрШаблон("Обрабатываем версию с комментом:'%1'", Комментарий));
		ГитРепозиторий.ПерейтиВВетку(СокрЛП(значение), Ложь , Истина);
		...
		Попытка
			...
			Лог.Информация(СтрШаблон("Объединяем %1 по правилу %2 ", Файлcf, ПутьКФайлуСравнению));
			// Лог.Отладка(СтрШаблон("Объединяем %1 по правилу %2 ", Файлcf, ПутьКФайлуСравнению));			
			УправлениеКонфигуратором.ОбъединитьКонфигурациюСФайлом(Файлcf, ПутьКФайлуСравнению);
