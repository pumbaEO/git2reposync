...
//
Функция УстановитьПутьКФайлуСравнения(Знач Путь) Экспорт
	ПутьКФайлуСравнению = Путь;
	Возврат ЭтотОбъект;
КонецФункции
...
Функция ПолучитьМассивВерсий(ГитРепозиторий, Фильтр)
	// ГитРепозиторий = ПолучитьГитРепозиторий(КаталогРабочейКопии);
	Параметры = Новый Массив();
	Параметры.Добавить("rev-parse");
	Параметры.Добавить("--short");
...
Процедура Синхронизировать(Знач КаталогРабочейКопии, Знач ПутьКХранилищу,
		Знач КоммитыДляСинхронизации) Экспорт
	...
	Лог.Информация("Подключение к хранилищу");
	ХранилищеКонфигурации.ПодключитьсяКХранилищу(Истина);
	ГитРепозиторий = ПолучитьГитРепозиторий(КаталогРабочейКопии);
	ТекущаяВетка = ПолучитьМассивВерсий(ГитРепозиторий, "HEAD")[0];
	МассвВерсий = ПолучитьМассивВерсий(ГитРепозиторий, КоммитыДляСинхронизации);
	ОбработчикПодписок.ПослеПолученияВерсийДляСинхронизации(КаталогРабочейКопии, МассвВерсий, КоммитыДляСинхронизации);
	Лог.Отладка("Массив версий");
	ТаблицаИсторииХранилища = ПрочитатьИзХранилищаИсториюКоммитовСАвторами(ПутьКХранилищу);
	
	Индекс = МассвВерсий.ВГраница();
	Пока Индекс >= 0 Цикл
		ПутьКФайлуСравнениюВрем = Новый Файл(ПутьКФайлуСравнению).ПолноеИмя;
		КаталогРабочейКопииВрем = КаталогРабочейКопии;
		Лог.Отладка("Каталог для єкспорта из edt в  xml:%1", КаталогРабочейКопииВрем);
		shacommit = МассвВерсий[Индекс];
		Лог.Информация(СтрШаблон("Обрабатываем версию :'%1'", shacommit));
		Индекс = Индекс - 1;
		Если Не ЗначениеЗаполнено(shacommit) Тогда
			Продолжить;
		КонецЕсли;
		
		Параметры = Новый Массив();
		Параметры.Добавить("show ");
		Параметры.Добавить("-s");
		Параметры.Добавить(shacommit);
		ГитРепозиторий.ВыполнитьКоманду(параметры);
		Комментарий = ГитРепозиторий.ПолучитьВыводКоманды();
		
		СтандартнаяОбработка = Истина;
		ОбработчикПодписок.ПередПроверкойВозможностиСинхронизацииКоммита(shacommit, Комментарий, ТаблицаИсторииХранилища, КаталогРабочейКопииВрем, СтандартнаяОбработка);
		
		Если СтандартнаяОбработка Тогда
			Если СтрНайти(Комментарий, "git2reposha:") Тогда
				Лог.Информация(Комментарий);
				Продолжить;
			КонецЕсли;
			ТакаяВерсияУжеЕст = Ложь;
			Для каждого Строка из ТаблицаИсторииХранилища Цикл
				Если СтрНайти(Строка.Комментарий, shacommit) Тогда
					ТакаяВерсияУжеЕст = Истина;
					Лог.Информация(СтрШаблон("Пропускаем версию %1, т.к. уже в хранилище %2 коммент:%3"
							, shacommit
							, Строка.НомерВерсии
							, Строка.Комментарий));
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ТакаяВерсияУжеЕст Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		Комментарий = Комментарий + Символы.ПС + "git2reposha:" + shacommit;
		Лог.Информация(СтрШаблон("Обрабатываем версию с комментом:'%1'", Комментарий));
		ГитРепозиторий.ПерейтиВВетку(СокрЛП(shacommit), Ложь, Истина);
		Файлcf = ВременныеФайлы.НовоеИмяФайла("cf");
		
		УправлениеКонфигуратором = ПолучитьУправлениеКонфигуратором();
		СтандартнаяОбработка = Истина;
		ОбработчикПодписок.ПередСборкойCFФайла(
			УправлениеКонфигуратором,
			КаталогРабочейКопииВрем,
			Файлcf,
			СтандартнаяОбработка);
		Если СтандартнаяОбработка Тогда
			УправлениеКонфигуратором.ЗагрузитьКонфигурациюИзФайлов(КаталогРабочейКопииВрем, , , Ложь);
			УправлениеКонфигуратором.ВыгрузитьКонфигурациюВФайл(Файлcf);
		КонецЕсли;
		
		СписокОбъектовДляЗахвата = Неопределено;
		РазрешатьЗахваченное = Истина;
		ОбработчикПодписок.ПриОпределениеОбъектовДляЗахватаВХранилище(СписокОбъектовДляЗахвата
			, РазрешатьЗахваченное
			, КаталогРабочейКопии);
		Лог.Отладка(СтрШаблон("Захватываем все объекты "));
		
		ХранилищеКонфигурации.ЗахватитьОбъектыВХранилище(СписокОбъектовДляЗахвата, РазрешатьЗахваченное);
		
		Попытка
			УправлениеКонфигуратором = ХранилищеКонфигурации.ПолучитьУправлениеКонфигуратором();
			Лог.Информация(СтрШаблон("Объединяем %1 по правилу %2 ", Файлcf, ПутьКФайлуСравнениюВрем));
			// Лог.Отладка(СтрШаблон("Объединяем %1 по правилу %2 ", Файлcf, ПутьКФайлуСравнению));
			УправлениеКонфигуратором.ОбъединитьКонфигурациюСФайлом(Файлcf, ПутьКФайлуСравнениюВрем);
			Лог.Информация(СтрШаблон("Помещанием с комментарием '%1' ", Комментарий));
			ХранилищеКонфигурации.ПоместитьИзмененияОбъектовВХранилище( , Комментарий);
		Исключение
	...
	ХранилищеКонфигурации.ОтменитьЗахватОбъектовВХранилище( , Истина);
	
	Лог.Информация("Вернемся на родительскую ветку:" + ТекущаяВетка);
	Лог.Отладка("Батько наш — Бандера, Україна — мати");
	ГитРепозиторий.ПерейтиВВетку(ТекущаяВетка);
	Лог.Отладка("Ми за Україну буде́м воювати!");
...
Процедура СинхронизироватьГитВХранилище(Знач КаталогРабочейКопии, Знач ПутьКХранилищу) Экспорт
...
Процедура СоздатьРасширениеВБазе(УправлениеКонфигуратором)
	...
	ПутьКШаблонуРасширения = ЗагрузчикЗапакованныхФайловgit2reposync.ПолучитьПутьКФайлу("tempExtension.cfe");
...
Функция ИмяЛога()
	...
	Возврат "oscript.lib.git2reposync";
